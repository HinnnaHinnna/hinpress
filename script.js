 ...
 *  - 바 컨테이너(.marquee-bar)에만 .shake 클래스를 붙였다 떼서
 *    barShake 키프레임을 한 번 재생시키는 방식
 */
if (marqueeBar) {
  // 한 번 "달그락" 흔들기
  function shakeBarOnce() {
    // 이미 흔들리는 중이면 중복 적용 방지
    ...
    // CSS에서 0.45s로 설정했으니, 약간 여유를 두고 클래스 제거
    setTimeout(() => {
...
function initPaddle() {
  ...
  // 바의 아랫면 y 좌표 (마퀴바를 어디에 두든 여기서 다시 계산)
...
if (marqueeBar) {
  ...
  /**
   * 🔹 모바일/태블릿용 터치 드래그 지원
   *  - 한 손가락으로 마퀴바를 좌우로 끌어서 움직일 수 있음
   */
  marqueeBar.addEventListener(
    'touchstart',
    (e) => {
...
// 공 여러 개 생성 (마퀴바 아래쪽 영역에서만 랜덤 배치)
for (let i = 0; i < numBalls; i++) {
  const radius = 16;

  // 마퀴바의 아랫면 기준으로 시작 위치 계산
